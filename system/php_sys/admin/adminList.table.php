        ...
        <table id="<?=$this->TableCssName?>" class="table table-striped table-bordered table-hover dataTable">
            ...
            <?php foreach( $Key_Arr as $key_v => $val_v ){ ?>
                ...
                ?>
                    ...
                    <td>
                    	<?php if( ( $Admin_data['Admin_ID'] != $val_v && $Admin_data['Group_Lv'] < $Value_Array['Group_Lv'][$key_v] ) && Menu_Use($Now_List, 'edit') ){ ?>
                        
                        <label>
                        	<input type="checkbox" class="ace ace-switch ace-switch-6 TableChange eshow" e-txt="直接點選更新" value="1" check-type="checkbox" check-id="<?=$val_v?>" check-field="<?=Turnencode($key_t)?>" <?=$Value?'checked="checked"':''?>>
                ...
                <?php } ?>
                    ...
                    <td>
                        ...
                        <?php if( $Operating['View'] ){ ?>
                            ...
                            <?php if( ( $Admin_data['Admin_ID'] != $val_v && $Admin_data['Group_Lv'] >= $Value_Array['Group_Lv'][$key_v] ) || !Menu_Use($Now_List, 'view') ){ ?>
                        ...
                        <?php if( $Operating['Delete'] ){ ?>
                            ...
                            <?php if( ( $Admin_data['Group_Lv'] >= $Value_Array['Group_Lv'][$key_v] ) || !Menu_Use($Now_List, 'delete') ){ ?>
                            <a class="grey" href="javascript:void(0)">
