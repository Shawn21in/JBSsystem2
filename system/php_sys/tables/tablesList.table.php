        ...
        <table id="<?=$this->TableCssName?>" class="table table-striped table-bordered table-hover dataTable">
            ...
            <?php foreach( $Key_Arr as $key_v => $val_v ){ ?>
                ...
                ?>
                    ...
                    <td>
                    	<?php if( $Admin_data['Admin_ID'] == 1 ){ ?>
                        
                        <label>
                        	<input type="checkbox" class="ace ace-switch ace-switch-6 TableCheck eshow" e-txt="直接點選更新" value="1" check-type="checkbox" check-id="<?=$val_v?>" check-field="<?=Turnencode($key_t)?>" <?=$Value?'checked="checked"':''?>>
