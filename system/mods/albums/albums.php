...
while( $row = $db->query_fetch() ){
	...
	$PathUrl = $Pathweb.$row[$Main_Key].'/';//圖片網址路徑
	...
	foreach( $table_option_arr as $tofield => $toarr ){
		...
		if( $toarr['TO_InType'] == 'uploadimg' && is_file($Path.$row[$Main_Key].DIRECTORY_SEPARATOR.$row[$tofield]) ){
			
			$Value_Array[$tofield.'_bUrl'][$sn] = $PathUrl.$row[$tofield];
			$Value_Array[$tofield.'_sUrl'][$sn] = $PathUrl.'sm_'.$row[$tofield];
		}
	}
